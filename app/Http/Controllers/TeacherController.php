...
class TeacherController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $data = \App\Helpers\CacheHelper::getDashboardStats('teacher', $userId, function () use ($teacher) {
            ...
            $formTeacherClasses = SchoolClass::with(['students' => function ($query) {
        ...
        return Inertia::render('teacher/TeacherDashboard', $data);
    }
    ...
    public function getClasses(Request $request)
    {
        ...
        $teacherAssignments = TeacherSubject::with(['schoolClass', 'subject'])
            ->where('teacher_id', $teacher->id)
            ->where('is_active', true)
            ->get()
            ->groupBy('class_id');
        ...
        $subjectClassIds = $teacherAssignments->keys();
        ...
        // Add subjects and student count to each class
        $classes = $classes->map(function ($class) use ($teacherAssignments, $formTeacherClassIds) {
            $class->student_count = $class->students->count();
            
            // Add subjects that teacher teaches in this class
            if ($teacherAssignments->has($class->id)) {
                $subjects = $teacherAssignments->get($class->id)
    ...
    public function getStudents(Request $request)
    {
        ...
        $classes = SchoolClass::whereIn('id', $subjectClassIds->merge($formTeacherClassIds)->unique())
                             ->where('is_active', true)
                             ->get();
        ...
        // Return JSON for API requests, Inertia for page requests
    ...
    public function addStudent(Request $request)
    {
        ...
        $student = Student::create([
            ...
            'password' => \Illuminate\Support\Facades\Hash::make('password'), // Hash the default password
            'is_active' => true,
    ...
    /**
     * Get students for a specific class and subject for attendance
     */
    public function getClassStudentsForAttendance(Request $request)
    {
        $teacher = $request->user();
        ...
        if (!$classId || !$subjectId) {
            return response()->json(['message' => 'Class ID and Subject ID are required'], 400);
        }
        ...
        if (!$isAssigned) {
            // Also check if teacher is form teacher of this class
            $isFormTeacher = SchoolClass::where('id', $classId)
                ->where('form_teacher_id', $teacher->id)
                ->where('is_active', true)
                ->exists();
