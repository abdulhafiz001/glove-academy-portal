...
class ScoreController extends Controller
{
    ...
    public function adminIndex(Request $request)
    {
        ...
        if (!$user->isAdmin()) {
            ...
            if ($user->isFormTeacher()) {
                ...
                if ($formTeacherClasses->isEmpty()) {
                    abort(403, 'No classes assigned as form teacher');
                }
                ...
                abort(403, 'Access denied. Only form teachers can view results.');
            }
        } else {
        ...
        if ($request->has('class_id')) {
            if (!$user->isAdmin()) {
                ...
                if (!$formTeacherClasses->contains($request->class_id)) {
                    abort(403, 'Access denied to this class');
                }
            }
        ...
        return Inertia::render('admin/Results', ['scores' => $scores]);
    }
    ...
    public function destroy(Request $request, Score $score)
    {
        ...
        if (!$user->isAdmin()) {
            ...
            // Also check if the score was created by this teacher
            $isScoreCreator = $score->teacher_id === $user->id;
            
            if (!$isFormTeacher && !$isSubjectTeacher && !$isScoreCreator) {
                return response()->json(['message' => 'You can only delete scores for students in your assigned classes or scores you created'], 403);
            }
        }
    ...
    public function adminStudentResults(Request $request, Student $student)
    {
        ...
        if (!$user->isAdmin() && $student->class_id) {
            ...
            if ($class->form_teacher_id === $user->id) {
                ...
                if (!$teacherAssignment) {
                    abort(403, 'You can only view results for students in your assigned classes');
                }
            }
        }
        ...
        return Inertia::render('admin/StudentResults', [
    ...
    public function teacherStudentResults(Request $request, Student $student)
    {
        ...
        if (!$teacher->isFormTeacher()) {
            abort(403, 'Access denied. Only form teachers can view student results.');
        }
        ...
        if (!$class) {
            abort(403, 'Access denied. You can only view results for students in classes where you are the form teacher.');
        }
        ...
        return Inertia::render('teacher/TeacherStudentResults', [
            'student' => $student->load('schoolClass'),
            'results' => $scores,
        ]);
    }
